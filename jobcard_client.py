...
#         """Asynchronously initialize job cards."""
...
#             logging.warning("Offline: Cannot fetch departments from MySQL")
...
#             logging.info(f"Loaded {len(self.departments)} departments from MySQL")
...
#             logging.error(f"UI update failed in {context}: {e}")
#             self.show_snack_bar(f"UI update failed in {context}: {e}", ft.Colors.RED_800)
...
#             logging.info(f"Loaded {len(self.job_cards)} job cards from SQLite for department {self.user_department}")
...
#         except sqlite3.Error as e:
#             logging.error(f"Error loading job cards: {e}")
...
#                     conn_sqlite.commit()
#                 except sqlite3.Error as e:
#                     logging.error(f"Error syncing job card {jc['job_number']}: {e}")
#                     self.show_snack_bar(f"Error syncing job card {jc['job_number']}: {e}", ft.Colors.RED_800)
#                     continue
#             logging.info(f"Synced {len(mysql_job_cards)} job cards from MySQL to SQLite for department {self.user_department}")
...
#         except mysql.connector.Error as e:
#             logging.error(f"Sync failed: Database error - {e}")
...
#                 conn_sqlite.commit()
#             conn_mysql.commit()
#             logging.info(f"Uploaded {len(job_cards)} job cards to MySQL")
...
#             else:
#                 logging.error(f"Upload failed: Database error - {e}")
...
#         if not self.is_online():
#             logging.warning("Offline: Cannot fetch entity info")
...
#         except mysql.connector.Error as e:
#             logging.error(f"Error fetching entity info: {e}")
...
#             logging.info("No job cards found, displaying empty table")
...
#         except Exception as e:
#             logging.error(f"Error opening job card details: {e}")
...
#             logging.info(f"Created job card {job_number} in SQLite")
...
#             logging.error(f"Error saving job card: {e}")
...
class JobCardPage(ft.Container):
    def __init__(self, page: ft.Page):
        ...
        # Set up page content without header and refresh button
        self.content = ft.Column(
            controls=[
                ft.Row(
                    controls=[self.status_filter, self.add_job_card_button, self.sync_button, self.upload_button],
